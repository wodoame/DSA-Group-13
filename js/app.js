...
// setting some local storage settings
...
const pushDetails = () => {
    ...
    let storedReservations = localStorage.getItem('app-reservations'); // get previously stored reservations
    // If there are stored reservations we parse the array of reservations otherwise we intialize an empty array
    storedReservations = storedReservations ? JSON.parse(storedReservations): []; 
    storedReservations.push(reservation); 
    storedReservations.forEach((res)=>{console.log(res);});
...
if(appSettings.displayPassengers){
    const reservations = new LinkedList(); // empty alphabetized linked list
    const storedReservations = JSON.parse(localStorage.getItem("app-reservations"));
    ...
    reservations.forEach((node)=>{console.log(node.value);});
    appSettings.displayPassengers = false; 
    localStorage.setItem("app-settings", JSON.stringify(appSettings));
}
